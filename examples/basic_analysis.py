...
def create_sample_data():
    ...
    # Create DataFrame
    df = pd.DataFrame({"epoch": timestamps, "values": rtt_values})
...
def main():
    ...
    config = JitterbugConfig(
        ...
        jitter_analysis=JitterAnalysisConfig(method="jitter_dispersion", threshold=0.3),
        verbose=True,
    )
    ...
    if congested_periods:
        ...
        for i, period in enumerate(congested_periods, 1):
            ...
            if period.latency_jump:
                print(
                    f"  Latency Jump: {period.latency_jump.has_jump} "
                    f"(magnitude: {period.latency_jump.magnitude:.1f}ms)"
                )

            if period.jitter_analysis:
                print(
                    f"  Jitter Change: {period.jitter_analysis.has_significant_jitter} "
