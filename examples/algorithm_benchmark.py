...
class DatasetGenerator:
    ...
    def generate_step_changes(
        ...
        for i, value in enumerate(values):
            ...
            measurements.append(
                RTTMeasurement(timestamp=timestamp, epoch=timestamp.timestamp(), rtt_value=value)
            )
    ...
    def generate_real_world_like(
        n_points: int = 200, congestion_periods: list[tuple] = None, noise_std: float = 2.0
    ) -> MinimumRTTDataset:
...
class AlgorithmBenchmark:
    ...
    def run_single_test(
        self, algorithm: str, dataset: MinimumRTTDataset, config_params: dict[str, Any] = None
    ) -> dict[str, Any]:
        ...
        config = ChangePointDetectionConfig(algorithm=algorithm, **config_params)
        ...
        try:
            ...
            memory_usage = sys.getsizeof(change_points) + sum(
                sys.getsizeof(cp) for cp in change_points
            )
    ...
    def print_summary(self):
        ...
        print("\n🏆 Algorithm Performance:")
        algo_stats = (
            successful_tests.groupby("algorithm")
            .agg(
                {
        ...
        print("\n📊 Dataset-Specific Performance:")
        dataset_performance = (
            successful_tests.groupby(["dataset_name", "algorithm"])
            .agg({"execution_time": "mean", "num_detected": "mean", "avg_confidence": "mean"})
            .round(4)
        )
    ...
    def generate_report(self, output_file: Path = None):
        ...
        <body>
            ...
            <div class="section">
                ...
                {
            self.results[self.results["success"]]
            .groupby("algorithm")
            .agg(
                {
