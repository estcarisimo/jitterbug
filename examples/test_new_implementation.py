...
def test_basic_functionality():
    ...
    try:
        ...
        for i in range(100):
            ...
            measurements.append(RTTMeasurement(timestamp=ts, epoch=ts.timestamp(), rtt_value=rtt))
        ...
        print(
            f"   ✓ Array conversion: {len(epochs)} epochs, RTT range: {rtt_values.min():.1f}-{rtt_values.max():.1f}ms"
        )
...
def test_io_functionality():
    ...
    try:
        ...
        if validation_results["valid"]:
            ...
            print(
                f"   ✓ RTT range: {metrics['rtt_statistics']['min']:.1f}-{metrics['rtt_statistics']['max']:.1f}ms"
            )
    ...
    try:
        ...
        custom_config = JitterbugConfig(
            change_point_detection=ChangePointDetectionConfig(
                algorithm="ruptures", threshold=0.3, min_time_elapsed=600
            ),
            jitter_analysis=JitterAnalysisConfig(method="ks_test", threshold=0.2),
            latency_jump=LatencyJumpConfig(threshold=0.8),
        )
        ...
        try:
            ChangePointDetectionConfig(threshold=2.0)  # Invalid threshold
