...
class JitterbugDashboard:
    ...
    def create_algorithm_comparison_report(
        ...
        self.plotter.plot_algorithm_comparison(
            dataset, algorithm_results, title=title, save_path=static_path
        )
        ...
        for algo, change_points in algorithm_results.items():
            comparison_stats[algo] = {
                ...
                "avg_confidence": sum(cp.confidence for cp in change_points) / len(change_points)
                if change_points
                else 0,
                "confidence_std": 0
                if len(change_points) <= 1
                    ...
                    sum(
                        (cp.confidence - comparison_stats[algo]["avg_confidence"]) ** 2
    ...
    def quick_visualize(
        ...
        if output_format == "static":
            ...
            fig = self.interactive.create_interactive_timeline(
                raw_data, min_rtt_data, results, change_points, title="Quick Interactive Analysis"
            )
            if save_path:
    ...
    def _calculate_statistics(
        self, results: CongestionInferenceResult, change_points: list[ChangePoint]
    ) -> dict[str, Any]:
