...
from ..models import ChangePoint, CongestionInferenceResult, MinimumRTTDataset, RTTDataset
...
class InteractiveVisualizer:
    ...
    def create_interactive_timeline(
        ...
        for inf in results.inferences:
            ...
            if inf.jitter_analysis:
                hover_text += (
                    f"Jitter Analysis: {inf.jitter_analysis.method}<br>"
        ...
        # Update layout
        fig.update_layout(
            title=title, height=height, showlegend=True, hovermode="x unified", template=self.theme
        )
        ...
        fig.update_yaxes(
            title_text="Congestion", row=3, col=1, tickvals=[0, 1], ticktext=["Normal", "Congested"]
        )
    ...
    def create_confidence_scatter(
        self, results: CongestionInferenceResult, title: str = "Confidence vs Duration Analysis"
    ) -> "go.Figure":
        """
        ...
        fig = go.Figure(
            data=go.Scatter(
                ...
                marker=dict(
                    size=10,
                    color=[
                        self.colors["congestion"] if s == "Congested" else self.colors["normal"]
                        for s in statuses
                    ],
                    opacity=0.7,
    ...
    def create_algorithm_comparison_plot(
        ...
        # Create subplots
        fig = make_subplots(
            rows=len(algorithm_results) + 1,
            cols=1,
            subplot_titles=["RTT Data"] + [f"{alg.upper()} Algorithm" for alg in algorithm_results],
            shared_xaxes=True,
    ...
    def create_dashboard(
        ...
        # Update layout
        fig.update_layout(title=title, height=800, showlegend=True, template=self.theme)
    ...
    def save_html(self, fig: go.Figure, filepath: Path, include_plotlyjs: bool = True) -> None:
        ...
        """
        fig.write_html(str(filepath), include_plotlyjs=include_plotlyjs, div_id="jitterbug-plot")
