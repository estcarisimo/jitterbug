...
class CongestionInferenceAnalyzer:
    ...
    def _apply_inference_logic(
        self, jump: LatencyJump, jitter: JitterAnalysis
    ) -> tuple[bool, float]:
        """
    ...
    def _post_process_inferences(
        self, inferences: list[CongestionInference]
    ) -> list[CongestionInference]:
        """
        ...
        for i in range(1, len(processed) - 1):
            ...
            if (
                current.is_congested != prev.is_congested
                and current.is_congested != next_inf.is_congested
            ):
