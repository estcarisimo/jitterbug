...
from ..models import ChangePoint as CoreChangePoint
from ..models import CongestionInference as CoreCongestionInference
from ..models import RTTMeasurement as CoreRTTMeasurement
...
class RTTMeasurementAPI(BaseModel):
    ...
    source: str | None = Field(None, description="Source IP address")
    destination: str | None = Field(None, description="Destination IP address")
...
class RTTDatasetAPI(BaseModel):
    ...
    metadata: dict[str, Any] | None = Field(None, description="Dataset metadata")
...
class JitterAnalysisAPI(BaseModel):
    ...
    threshold: float = Field(..., description="Detection threshold")
    p_value: float | None = Field(None, description="Statistical p-value if applicable")
...
class CongestionInferenceAPI(BaseModel):
    ...
    confidence: float = Field(..., ge=0, le=1, description="Confidence score")
    latency_jump: LatencyJumpAPI | None = Field(None, description="Latency jump analysis")
    jitter_analysis: JitterAnalysisAPI | None = Field(None, description="Jitter analysis")
...
class AnalysisRequestAPI(BaseModel):
    ...
    config: dict[str, Any] | None = Field(None, description="Analysis configuration")
    algorithm: str | None = Field("ruptures", description="Change point detection algorithm")
    method: str | None = Field("jitter_dispersion", description="Jitter analysis method")
    threshold: float | None = Field(0.25, description="Detection threshold")
    min_time_elapsed: int | None = Field(1800, description="Minimum time between change points")
...
class AnalysisResponseAPI(BaseModel):
    ...
    result: AnalysisResultAPI | None = Field(None, description="Analysis results")
...
class AlgorithmComparisonRequestAPI(BaseModel):
    ...
    config: dict[str, Any] | None = Field(None, description="Common configuration")
...
class AlgorithmComparisonResponseAPI(BaseModel):
    ...
    results: dict[str, list[ChangePointAPI]] | None = Field(
        None, description="Results by algorithm"
    )
    performance: dict[str, float] | None = Field(None, description="Performance metrics")
    error: str | None = Field(None, description="Error message if failed")
...
def convert_to_api_model(
    core_obj: CoreRTTMeasurement | CoreCongestionInference | CoreChangePoint,
) -> RTTMeasurementAPI | CongestionInferenceAPI | ChangePointAPI:
...
def convert_from_api_model(
    api_obj: RTTMeasurementAPI | CongestionInferenceAPI | ChangePointAPI,
) -> CoreRTTMeasurement | CoreCongestionInference | CoreChangePoint:
