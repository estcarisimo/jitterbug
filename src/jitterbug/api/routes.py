...
from ..models import JitterbugConfig, MinimumRTTDataset, RTTDataset
from .models import (
    AlgorithmComparisonRequestAPI,
    AlgorithmComparisonResponseAPI,
    AnalysisRequestAPI,
    AnalysisResponseAPI,
    AnalysisResultAPI,
    ErrorResponseAPI,
    HealthCheckAPI,
    StatusAPI,
    ValidationRequestAPI,
    ValidationResponseAPI,
    convert_from_api_model,
...
async def health_check():
    ...
    try:
        ...
        try:
            import ruptures  # noqa: F401  (availability probe)
...
async def get_status():
    ...
    try:
        avg_response_time = (
            ...
            if service_stats["response_times"]
            else 0
        )
...
async def analyze_data(
    ...
    try:
        ...
        core_measurements = [
            convert_from_api_model(measurement) for measurement in request.data.measurements
        ]
...
async def validate_data(
    ...
    try:
        ...
        return ValidationResponseAPI(valid=False, errors=[error_msg], warnings=[], metrics={})
...
async def compare_algorithms(
    ...
    try:
        ...
        min_rtt_data = MinimumRTTDataset(measurements=core_measurements, interval_minutes=15)
        ...
        for algorithm in request.algorithms:
            try:
                ...
                logger.info(
                    f"Algorithm {algorithm}: {len(change_points)} change points in {algo_time:.2f}s"
                )
...
async def value_error_handler(request: Request, exc: ValueError):
    ...
    return JSONResponse(
        ...
        content=ErrorResponseAPI(
            error="ValidationError", message=str(exc), timestamp=datetime.now()
        ).dict(),
    )
