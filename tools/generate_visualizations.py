...
def generate_algorithm_visualization(algorithm_name, output_dir):
    ...
    try:
        ...
        def epoch_to_datetime(epoch_series):
            return [datetime.fromtimestamp(t, tz=timezone.utc) for t in epoch_series]
        ...
        axes[0].plot(
            raw_times, raw_df["values"], color="C0", alpha=0.7, linewidth=1, label="Raw RTT"
        )
        axes[0].set_ylabel("RTT (ms)", fontsize=12)
        axes[0].set_title(
            f"Network Congestion Analysis - {algorithm_name.upper()} Algorithm",
            fontsize=14,
            fontweight="bold",
        )
        ...
        axes[1].plot(
            mins_times, mins_df["values"], color="C1", alpha=0.8, linewidth=2, label="Minimum RTT"
        )
        ...
        for inference in results.inferences:
            start_dt = datetime.fromtimestamp(inference.start_epoch, tz=timezone.utc)
            end_dt = datetime.fromtimestamp(inference.end_epoch, tz=timezone.utc)
...
def create_comparison_chart(results, output_dir):
    ...
    for bar, value in zip(bars1, detected_periods, strict=True):
    ...
    for bar, accuracy in zip(bars2, accuracies, strict=True):
        height = bar.get_height()
        ax2.annotate(
            f"{accuracy:.1f}%",
            xy=(bar.get_x() + bar.get_width() / 2, height),
...
def main():
    ...
    with readme_path.open("w") as f:
        ...
        f.write(
            "This directory contains visualization examples for both change point detection "
        ...
        for algo in algorithms:
            f.write(
                f"- `{algo}_congestion_analysis.png` - Visualization for {algo.upper()} algorithm\n"
            )
