...
class TestRTTMeasurement:
    ...
    def test_invalid_rtt_value(self):
        ...
        with pytest.raises(ValueError, match="greater than 0"):
            RTTMeasurement(timestamp=timestamp, epoch=timestamp.timestamp(), rtt_value=-1.0)
...
class TestRTTDataset:
    ...
    def create_sample_measurements(self, count=10):
        ...
        for i in range(count):
            ...
            measurements.append(
                RTTMeasurement(
                    timestamp=timestamp, epoch=timestamp.timestamp(), rtt_value=20.0 + i * 0.5
                )
            )
...
class TestChangePoint:
    ...
    def test_valid_change_point(self):
        ...
        timestamp = datetime.now()
        cp = ChangePoint(
            timestamp=timestamp, epoch=timestamp.timestamp(), confidence=0.85, algorithm="ruptures"
        )
...
class TestJitterbugConfig:
    ...
    def test_custom_config(self):
        ...
        config = JitterbugConfig(
            change_point_detection=ChangePointDetectionConfig(algorithm="bcp", threshold=0.3),
            jitter_analysis=JitterAnalysisConfig(method="ks_test", threshold=0.15),
            verbose=True,
        )
