...
class TestDataGenerator:
    ...
    def generate_synthetic_data(
        ...
        for i in range(num_points):
            ...
            measurements.append(
                RTTMeasurement(
                    timestamp=timestamp, epoch=timestamp.timestamp(), rtt_value=rtt_value
                )
            )
...
class TestRupturesDetector:
    ...
    def test_ruptures_detector_initialization(self):
        ...
        config = ChangePointDetectionConfig(
            algorithm="ruptures", threshold=0.25, ruptures_model="rbf", ruptures_penalty=10.0
        )
    ...
    def test_ruptures_different_models(self):
        ...
        for model in models:
            try:
                config = ChangePointDetectionConfig(
                    algorithm="ruptures", ruptures_model=model, ruptures_penalty=10.0
                )
                detector = RupturesDetector(config)
    ...
    def test_ruptures_penalty_effects(self):
        ...
        try:
            ...
            config_low = ChangePointDetectionConfig(algorithm="ruptures", ruptures_penalty=5.0)
            detector_low = RupturesDetector(config_low)
            cp_low = detector_low.detect(dataset)

            # High penalty (fewer change points)
            config_high = ChangePointDetectionConfig(algorithm="ruptures", ruptures_penalty=20.0)
...
class TestBayesianChangePointDetector:
    ...
    def test_bcp_detector_initialization(self):
        ...
        config = ChangePointDetectionConfig(algorithm="bcp", threshold=0.25)
    ...
    def test_bcp_threshold_effects(self):
        ...
        try:
            ...
            config_low = ChangePointDetectionConfig(algorithm="bcp", threshold=0.1)
            detector_low = BayesianChangePointDetector(config_low)
            cp_low = detector_low.detect(dataset)

            # High threshold (less sensitive)
            config_high = ChangePointDetectionConfig(algorithm="bcp", threshold=0.5)
    ...
    def test_bcp_error_handling(self):
        ...
        measurements = [
            RTTMeasurement(
                timestamp=datetime.now(), epoch=datetime.now().timestamp(), rtt_value=20.0
            )
        ]
...
class TestAlgorithmComparison:
    ...
    def test_algorithm_consistency(self):
        ...
        for algorithm in algorithms:
            try:
                config = ChangePointDetectionConfig(algorithm=algorithm, threshold=0.25)
                detector = ChangePointDetector(config)
        ...
        for _algo, cps in results.items():
    ...
    def test_algorithm_performance_characteristics(self):
        ...
        for _algo, perf in performance.items():
